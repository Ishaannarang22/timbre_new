...
exactly the voice configured in .env. The motivational quote is generated by Nemotron and woven
into the call.
...
# companion, not a morning wake-up. Spoken verbatim exactly once, same mechanism as the
...
def inbound_system_prompt() -> str:
    # INBOUND persona: Ishaan dialed US. A warm, concise phone companion — NO motivational
    # quote, NO morning framing. The opening line (INBOUND_GREETING) is spoken deterministically
    # once and seeded as the assistant's first turn, so — exactly like the outbound flow — the
    # model must NOT greet again.
    return (
...
class GoodbyeProcessor(FrameProcessor):
    ...
    def __init__(self, quote: str, mode: str = "outbound"):
        ...
        self._quote = quote
        # mode == "inbound" means Ishaan dialed us (companion persona, NO motivational
...
async def twiml(request: Request) -> PlainTextResponse:
    ...
    # is a companion persona with NO quote, so we skip that NVIDIA call (less latency, no
    ...
    # So the URL is clean, and ALL custom data (per-call token + persona mode) is passed via
    # <Parameter> children, which Twilio echoes in the 'start' event's start.customParameters
    # — the officially-supported channel that /ws reads.
    # https://www.twilio.com/docs/voice/twiml/stream#custom-parameters
    ws_url = f"wss://{host}/ws"
    logger.info(
        f"/twiml call_sid={call_sid} mode={mode} -> stream {ws_url} (token via <Parameter>)"
    )
    xml = (
...
async def ws(websocket: WebSocket) -> None:
    ...
    # somehow lost). INBOUND has no quote — the companion persona never delivers one.
    if mode == "outbound":
    ...
    logger.info(
        f"/ws connected stream_sid={stream_sid} call_sid={call_sid} mode={mode}"
    )
    ...
    # The persona is the morning-quote flow (outbound) or the companion flow (inbound).
    ...
    prompt = inbound_system_prompt() if mode == "inbound" else system_prompt(quote)
